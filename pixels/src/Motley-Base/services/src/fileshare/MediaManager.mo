...
shared ({ caller }) actor class MediaManager( admins : [Principal]) = this {
  ...
  // stable var _dependants : Text.Set         = Text.Set.init();
  ...
  // stable var installer = Principal.fromText("yw67o-4aaaa-aaaal-abwga-cai");
  stable var installer = caller;
  
  installer := Principal.fromText("yw67o-4aaaa-aaaal-abwga-cai");
  _devices := DQ.empty();

  public shared query func last_report() : async Text { _lastreport };

  public shared ({caller}) func write_request( request : Request, save : SaveCmd, secret : Blob ) : async Return<Index> {
    assert Principal.equal(caller, installer);
  ...
  public shared ({caller}) func clock() : () {
    if ( Nat.rem(_tick, 1) == 0 ){
      ...
      while ( count_devices() < _MAX_RESERVE_ ){
        let new : Device = await new_device();
        _devices := DQ.pushFront<Device>(_devices, new);
      };
  ...
  func count_devices() : Nat {
    var count : Nat = 0;
    var backup = DQ.empty<Device>();
    while ( not DQ.isEmpty<Device>(_devices) ){
      switch( DQ.popFront<Device>(_devices) ){
        case null assert false;
        case ( ?(device, dq) ){
          _devices := dq;
          backup := DQ.pushBack<Device>(backup, device);
          count += 1;
        };
      };
    };
  ...
  public shared ({caller}) func chain_report( cr : T.ChainReport ) : () {
    switch cr {
      case ( #ping ){
        ...
        let chsvc : ChildService = actor(Principal.toText(installer));
        chsvc.chain_report(
          #report({
  ...
  func release( device : Device ) : () {
    _devices := DQ.pushFront<Device>(_devices, device); // Orig pushFront
    // if ( device.available == T.SectorCount.blocks(1) ){
    //   _reserves := DQ.pushFront<Device>(_reserves,device); // Orig pushFront
    //   _rescount += 1;
    // } else {
    //   _devices := DQ.pushFront<Device>(_devices, device); // Orig pushFront
    // };
  ...
  func spawn_daemon( index : Index, request : Request, save : SaveCmd, secret : Blob ) : () {
