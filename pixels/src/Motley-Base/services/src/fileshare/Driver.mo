...
import Hash "../../../Motley-Base/src/encoding/sha224";
...
module {
  ...
  public class DeviceDriver(
    ...
    let final_files = TM.TrieMap<Index,T.File>(Index.equal, Index.hash);
    let inode_to_file = TM.TrieMap<Index,[T.TempFile]>(Index.equal, Index.hash);
    let device_to_inodes = TM.TrieMap<Principal,[(T.BlockCount,Index)]>(Principal.equal, Principal.hash);
    let distributed_inodes = TM.TrieMap<Index,([T.StorageDevice],T.BlockCount)>(Index.equal, Index.hash);
    ...
    public func all_formatted() : Bool {
      let test = TrieSet.fromArray<Index>(
        Iter.toArray<Index>(final_files.keys()), Index.hash, Index.equal
      );
      for ( entry in inode_to_file.keys() ){
        if ( not TrieSet.mem<Index>(test, entry, Index.hash(entry), Index.equal) ){
          return false;
        };
      };
      return true;
