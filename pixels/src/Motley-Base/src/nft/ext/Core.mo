...
module ExtCore = {
  ...
  public type Metadata = { #nonfungible : { metadata : ?Blob } };
  ...
  public type BulkNotifyCallback = shared ([TokenIdentifier], User, Balance, Memo) -> async ?Balance;
  public type BulkNotifyService = actor { bulkTokenTransferNotification : BulkNotifyCallback };
  ...
  public type Service = actor {
    ...
    transferBulk: shared (request : BulkTransferRequest) -> async TransferResponse;

    getRegistry: shared () -> async [(TokenIndex,AccountIdentifier)];

    tokens: shared (AccountIdentifier) -> async Result.Result<[TokenIndex],CommonError>;
  };
  ...
  public module TokenIdentifier = {
    ...
    public func fromBytes(c : [Nat8], i : TokenIndex) : TokenIdentifier {
      ...
      return Principal.toText(Principal.fromBlob(Blob.fromArray(bytes)));
    };
  ...
  //     return Hex.encode(Array.append(c, bytes));
  //   };
  //   //Coz can't get principal directly, we can compare the bytes
  //   public func isPrincipal(tid : TokenIdentifier, p : Principal) : Bool {
  //     let tobj = decode(tid);
