...
actor class MoonWalker() = this {
  ...
  private var salesFees : [(AccountIdentifier, Nat64)] = [
    ("07ed96a84229a40439a68054bf8d6ac4e9fb472ea6c01520cb46a68eb2faf6f4", 5000), //Royalty Fee
  ...
  private stable var _minter : Principal  = Principal.fromText("jdfjg-amcja-wo3zr-6li5k-o4e5f-ymqfk-f4xk2-37o3d-2mezb-45y3t-5qe");
  ...
  func _getNextSubAccount() : SubAccount {
    var _saOffset = 4294967296 + 30000;
    _nextSubAccount += 1;
  ...
  public query func get_royalty_address(): async Text { salesFees[0].0 };
  ...
  public shared(msg) func adminKillHeartbeat() : async () {
    // assert(msg.caller == _minter or msg.caller == Principal.fromText("55rjt-fww5v-t6ktt-etpvr-wmwn2-t5eks-w365d-zwjbe-yi2ux-uapb4-hae"));
    assert(msg.caller == _minter or msg.caller == Principal.fromText("6pbsk-5kqts-mrulc-qb3gr-gyh7c-n55ms-p773q-457bq-j4zbl-c3a5f-bae"));
    _runHeartbeat := false;
  };
...
public query func heartbeat_pending() : async [(Text,Nat)]
  {
    ...
    ("Settlements", unlockedSettlements().size())
    ];
  };
  ...
  public shared(msg) func addThumbnail(handle : AssetHandle, data : Blob) : async () {
    assert(msg.caller == Principal.fromText("aba3c-mjh2y-sp47u-cpa7l-fsiga-5dg2p-wqubj-ecp7c-zwrvc-724ko-wae"));
  ...
  public query(msg) func getAssets() : async [(AssetHandle, Asset)] {
    //  assert(msg.caller == Principal.fromText("aba3c-mjh2y-sp47u-cpa7l-fsiga-5dg2p-wqubj-ecp7c-zwrvc-724ko-wae"));
    Iter.toArray(_assets.entries());
  };

  public query(msg) func getThumbs() : async [(AssetHandle)] {
    //  assert(msg.caller == Principal.fromText("aba3c-mjh2y-sp47u-cpa7l-fsiga-5dg2p-wqubj-ecp7c-zwrvc-724ko-wae"));
     var resp : [(AssetHandle)] = [];
         for(e in _assetThumb.entries()){
      resp := Array.append(resp, [e.0]);
    };
    resp
  };
  ...
  public query func http_request(request : HttpRequest) : async HttpResponse {
      ...
      case (?tokenid) {
        switch(_getTokenIndex(tokenid)) {
          case (?index) {
              ...
              case(?t) {
                if (t == "thumbnail") {
                  return {
                    ...
                    body = Option.unwrap(_assetThumb.get(Nat32.toText(index)));
            ...
            switch(_assets.get(Nat32.toText(index))) {
              case(?asset)  {
                return {
                  status_code = 200;
                  headers = [("content-type", "image/svg+xml")];
                  body = Text.encodeUtf8 ("<svg width=\"2480\" height=\"3508\" xmlns=\"http://www.w3.org/2000/svg\"><image href=\"https://"#asset.canister#".raw.ic0.app/?index="#Nat32.toText(asset.id)#"\" width=\"2480\" height=\"3508\"/></svg>");
                  streaming_strategy = null;
                };
              };
              case (_){
    ...
    switch(_getParam(request.url, "index")) {
      case (?index) {
          ...
          case(?t) {
            if (t == "thumbnail") {
              return {
                status_code = 200;
                headers = [("content-type", "image/jpeg"), ("cache-control", "public, max-age=15552000")];
                body = Option.unwrap(_assetThumb.get(Nat32.toText(_textToNat32(index))));
                streaming_strategy = null;
              };
            };
          };
        ...
        switch(_assets.get(Nat32.toText(_textToNat32(index)))) {
          case(?asset)  {
            return {
              status_code = 200;
              headers = [("content-type", "image/svg+xml")];
              body = Text.encodeUtf8 ("<svg width=\"2480\" height=\"3508\" xmlns=\"http://www.w3.org/2000/svg\"><image href=\"https://"#asset.canister#".raw.ic0.app/?index="#Nat32.toText(asset.id)#"\" width=\"3508\" height=\"3508\"/></svg>");
              streaming_strategy = null;
            };
          };
          case (_){
...
func _getSubAccountForAddress(a : AccountIdentifier) : ?SubAccount {
    ...
    while(s < (4294967296 + _nextSubAccount+ 30000)) {
...
public query func getNextSubAccount () : async Nat
{
  _nextSubAccount;
};
...
public shared(msg) func adminRefund(password : Text, escrow : AccountIdentifier, sendto : AccountIdentifier) : async Text {
    assert(password == "SET_PASSWORD_HERE");
    ...
    switch(_getSubAccountForAddress(escrow)){
      case(?sa) {
        let response : ICPTs = await LEDGER_CANISTER.account_balance_dfx({account = escrow});
