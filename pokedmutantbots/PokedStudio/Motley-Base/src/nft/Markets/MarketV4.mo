...
module Market {
  ...
  public type FireSale = {
    timeouts       : Nat;
    var delist     : Bool;
    var buyers     : Principal.Tree<MultiLock>;
    var status     : { #idle; #busy };
  };
  ...
  public func stats( m : Market ) : Statistics {
    assert m.init;

    let total_txns : Nat64 = Nat64.fromNat(m.transactions.size);
    var total_listings : Nat = 0;
    var total_sales : Nat64 = 0;

    if ( total_txns > 0 ){ 
      for ( index in Iter.range(0, Nat64.toNat(total_txns)-1)){
        total_sales += m.transactions.elems[index].price
      }
    };

    if ( m.range > 0 ){
      for ( index in Iter.range(0,m.range-1)){
        switch( m.listings[index] ){
          case ( #listed _ ) total_listings += 1;
          case _ ();
        }
      }
    };
    ...
    return {
      ...
      volume = total_sales;
      average = total_sales / total_txns;
    };
        ...
        switch( m.locks[i] ){
          ...
          case ( #firesale fs ){
            switch( fs.status ){
              ...
              case ( #idle ){
                fs.status := #busy;
                let fslocks : Iter.Iter<MultiLock> = Principal.Tree.vals<MultiLock>(fs.buyers);
                for ( fslock in fslocks ) m.refunds := DQ.pushBack<Refund>(m.refunds, (fslock.0,fslock.1));
                m.listings[i] := #delisted( Time.now() );
      ...
      case ( #listed listing ){
        ...
        switch( m.locks[i] ){
          ...
          case ( #firesale fs ){
            ...
            if ( Option.isSome(Principal.Tree.find<MultiLock>(fs.buyers, c)) ) return #err(#Locked)
            else {

              let _sa : SubAccount = nat_to_subaccount( m.next_sa );
              fs.buyers := Principal.Tree.insert<MultiLock>(
                fs.buyers, c, (b, _sa, Buffer.toArray<(AccountId,Nat64)>(_fees))
              );
          ...
          case ( #unlocked timeouts ) {
            ...
            if ( timeouts < m.fs_threshold ){
              ...
              m.locks[i] := #firesale({
                timeouts = timeouts;
                var delist = false;
                var buyers = Principal.Tree.fromEntries<MultiLock>(
                  [ (c, (b, _sa, Buffer.toArray( _fees ))) ]
                );
                var status = #idle;
              });
              #ok( AccountId.fromPrincipal(m.escrow, ?_sa) )
      ...
      case ( #firesale fs ){
        ...
        switch( fs.status ){
          ...
          case ( #idle ){
            ...
            if ( Option.isNull(caller) and not bypass ) return #err(#Busy);
            let claimant : Principal = Option.get<Principal>(caller, Principal.placeholder());
            ...
            switch( m.listings[i] ){
              ...
              case ( #listed listing ){
                ...
                if bypass {
                  ...
                  let empty = Buffer.Buffer<(Principal,MultiLock)>(0);

                  let pending = Buffer.fromIter<(Principal,MultiLock)>(
                    Principal.Tree.entries<MultiLock>( fs.buyers )
                  );

                  fs.buyers := Principal.Tree.init<MultiLock>();

                  for ( (caller, lock) in pending.vals() ){

                    if paid empty.add((caller, lock))
                    else {

                      try {
                        let balance = await Ledger.account_balance(
                          AccountId.fromPrincipal(m.escrow, ?lock.1)
                        );
                    
                        if ( balance.e8s < listing.price ) empty.add((caller, lock))
                        else {
                          ...
                          // Add any locks that came in during the previous await calls
                          empty.append( Buffer.fromIter<(Principal,MultiLock)>(
                            Principal.Tree.entries<MultiLock>( fs.buyers )
                          ));
                      ...
                      } catch (e) empty.add((caller, lock))
                    }
                  };
                
                  if paid {

                    for ( (caller, lock) in empty.vals() ){
                      m.refunds := DQ.pushBack<Refund>(m.refunds, (lock.0, lock.1))
                    };
                    ...
                    fs.status := #idle;
                    for ( (caller, lock) in empty.vals() ){
                      fs.buyers := Principal.Tree.insert<MultiLock>(fs.buyers, caller, lock);
                    };
                  ...
                  switch( Principal.Tree.find<MultiLock>(fs.buyers, claimant) ){

                    case null { fs.status := #idle; #err(#NotLocked) };

                    case ( ?lock ){

                      try {

                        let balance = await Ledger.account_balance( AccountId.fromPrincipal(m.escrow, ?lock.1) );
                        ...
                        if ( balance.e8s < listing.price ) {
                          ...
                          fs.buyers := Principal.Tree.delete<MultiLock>(fs.buyers, claimant);

                          for ( remlock in Principal.Tree.vals<MultiLock>(fs.buyers) ){
                            m.refunds := DQ.pushBack<Refund>(m.refunds, (remlock.0, remlock.1))
                          };
                        ...
                        fs.status := #idle;
                        #err(#Fatal("Canister Rejected; Ledger trapped"))
  ...
  public func refund( m : Market ) : async () {
    ...
    label l loop {
      ...
      try {
        ...
        if ( balance > 0 ) add_disbursement(m, (0, buyer, Blob.fromArray(subaccount), balance));
