...
import CRC "../../../../Motley-Base/src/encoding/crc32";
...
shared ({ caller = _installer }) actor class BlockDevice( admin : Principal ) = this {
  ...
  public shared ({caller}) func write( sindex : T.Index, bindex : T.Index, data : Blob, crc : Text ) : () {
    ...
    if ( Text.contains(crc2, #text(crc)) ){current.1[bindex] := data}
  ...
  func stage( request : T.FormatRequest ) : Iter.Iter<(T.Index,T.Mapping,Text)> {
    ...
    Iter.fromArray<(T.Index,T.Mapping,Text)>( file_buffer.toArray() );
  };
