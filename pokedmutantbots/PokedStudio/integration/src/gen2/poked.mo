...
import FSTypes2 "../../../Motley-Base/src/asset/Types";
...
shared ({ caller = _installer }) actor class NFT_Registry() = this {
  ...
  public query ({caller}) func fileshareId(): async Principal {
    assert _is_admin(caller);
    _fileshare
  };

  public query ({caller}) func export_filesystem(): async Return<Mount, FSTypes2.Error> {
    assert _is_admin(caller);
    Filesystem.export(_mount, "/", _self, null, null)
  };
  ...
  public shared query func fees() : async (Nat64, Nat64, Nat64) {
    (_market.fees.min_fee, _market.fees.max_fee, _market.fees.royalty)
  };

  public shared ({caller}) func set_fees( fees: (Nat64, Nat64, Nat64) ): async () {
    assert _is_admin(caller);
    _market.fees.min_fee := fees.0;
    _market.fees.max_fee := fees.1;
    _market.fees.royalty := fees.2;
  };
...
public query func heartbeat_pending() : async [(Text, Nat)] {
    let d = Market.query_disbursements(_market);
    let size = Iter.size(d.vals()); // Alternative to Array.size
    return [
        ("Disbursements", size)
    ];
};
  ...
  public shared ({caller}) func mint_nft( request : MintRequest ) : async ?Index {
    assert _is_admin(caller);
    if ( not Path.is_valid( request.path ) ) return null;
    let receiver : AccountId = AccountId.fromPrincipal(request.receiver, null);
    var metadata : Tokens.Metadata.Metadata = Tokens.Metadata.init();
    for ( dentry in Filesystem.root_folder( _mount ).vals() ){
      switch( _walk( request.path ) ){
    ...
    let token : Index = Tokens.mint_token(_tokens, receiver, request.path, metadata);
  ...
  public shared ({caller}) func get_royalty_address(): async AccountId { _market.royalty };

  public shared ({caller}) func set_royalty_address(aid: AccountId): async Return<(),Text> {
    assert _is_admin(caller);
    if ( AccountId.valid(aid) == false ) return #err("Invalid Account ID")
    else #ok( _market.royalty := aid );
  };
  ...
  public shared ({caller}) func process_disbursements() : () {
    ignore Market.disburse( _market );
  };

  public shared ({caller}) func process_refunds() : () {
    ignore Market.refund( _market );
  };
  ...
  public shared query ({caller}) func check_metadata(i: Index, key : Text): async Tokens.Metadata.Value {
    Tokens.query_metadata(_tokens, i, key)
  };
  ...
  public query func areTokensLocked(tokens : [Index] ): async Return<[Index],CommonError> {
    _are_tokens_locked(tokens);
  };
