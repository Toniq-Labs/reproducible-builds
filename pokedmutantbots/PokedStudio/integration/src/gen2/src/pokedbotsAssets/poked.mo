...
actor class Canister() = this {
  ...
  // _supply := 0;
  // _nextTokenId := 0;
  // _registry := TrieMap.TrieMap<TokenIndex,AccountIdentifier>(ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash);
  // _tokenMetadata := TrieMap.TrieMap<TokenIndex,Metadata>(ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash);
  // _owners := TrieMap.TrieMap<AccountIdentifier,[TokenIndex]>(AID.equal, AID.hash);
  ...
  public query func getTokenId( n : Nat32 ) : async TokenIdentifier {
  ...
  public shared func reset_test() : () {
    for ( i in Iter.range(0,27)){
      _transferTokenToUser(Nat32.fromNat(i), "c34ae19f483afc1c44c54e6712443033260b5a09a5af2a6cbfdc23a3657c8ec6");
    };
  };
